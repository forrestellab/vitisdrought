...
combined_final_data<- read.csv("data_output/anova/species_final_anova_data_outremoved_1")
combined_final_data<- combined_final_data %>%
  select(-starts_with("mean_"), -starts_with("sd_")) %>%
  select(-c(Treatment_factor, PD))%>%
  mutate(LWP = -LWP)   %>%
  distinct()
...
tukey_columns <- grep("^Tukey_", colnames(combined_final_data), value = TRUE)

# Reshape the data to long format for Tukey values
tukey_data <- combined_final_data %>%
...
variable_labels <- c(
  ...
  `gsw` = "g[s]~~~(mol~m^-2~s^-1)",
  `Tleaf` = "T[leaf]~~~(~C)",
  `intrWUE_AE` = "A/E~~(µmol~CO[2]~mol^{-2}~H[2]~O)", 
  `instWUE_AGs` = "A[n]/g[s]~~(µmol~CO[2]~mol^{-2}~H[2]~O)", 
  `LWP` = "Psi[leaf]~~~(MPa)"
)
...
#all n=3-10 for photysynthesis measurements except for 