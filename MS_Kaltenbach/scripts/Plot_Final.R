...
combined_fin_data<- read.csv("data_output/anova/species_final_anova_data_outremoved")

symnum.args = list(cutpoints = c(0, 0.05, 1), symbols = c("*", "ns"))
# Plot individual --------------------------------------------------------------------
A_box_plot <- ggplot(combined_fin_data, aes(x = paste(Species, Genotype), y = A, fill = Treatment)) +
  geom_boxplot() +
  theme_bw() +
  theme(
  ...
  geom_text(data = combined_fin_data %>% 
...
str(combined_fin_data)
# bebbe -------------------------------------------------------------------
plots <- list()

# Columns to plot
columns <- c("A", "Ci", "E", "gsw", "E", "Tleaf", "intrWUE_AE", "instWUE_AGs", "LWP", "PD" )

# Custom y-axis labels
labels <- c("A", "Ci", "E", "gsw", "E", "Tleaf", "intrWUE_AE", "instWUE_AGs", "LWP", "PD" )
...
for (i in seq_along(columns)) {
  ...
  # Create the plot
  plot <- ggplot(combined_fin_data, aes(x = paste(Species, Genotype), y = .data[[col]], fill = Treatment)) +
    geom_boxplot() +
    theme_plot +
    xlab("") +
...
final_plot_star <- final_plot + theme(plot.margin = margin(0, 0.0, 0.0, 0.0, "cm"))

ggsave("final_plot_labels_star.pdf", final_plot_star, width = 20, height = 13, units = "cm")
...
for (i in seq_along(columns)) {
  ...
  filtered_data <- combined_fin_data[complete.cases(combined_fin_data[, c("Species", "Genotype", col, "Treatment")]), ]
...
ggsave("final_data_plot_labels2_star.pdf", final_plot, width = 20, height = 103, units = "cm")
