...
combined_mid_data<- read.csv("data_output/anova/species_middle_anova_data_outremoved")

symnum.args = list(cutpoints = c(0, 0.05, 1), symbols = c("*", "ns"))

# Plot individual --------------------------------------------------------------------
A_box_plot <- ggplot(combined_mid_data, aes(x = paste(Species, Genotype), y = A, fill = Treatment)) +
  geom_boxplot() +
  theme_bw() +
  theme(
  ...
  geom_text(data = combined_mid_data %>% 
...
labels <- c("A", "Ci", "E", "gsw", "Tleaf", "intrWUE_AE", "instWUE_AGs", "LWP" )
...
for (i in seq_along(columns)) {
  ...
  # Create the plot
  plot <- ggplot(combined_mid_data, aes(x = paste(Species, Genotype), y = .data[[col]], fill = Treatment)) +
    geom_boxplot() +
    theme_plot +
    xlab("") +
...
ggsave("middle_plot_labels_star.pdf", final_plot, width = 20, height = 103, units = "cm")
...
for (i in seq_along(columns)) {
  ...
  filtered_data <- combined_mid_data[complete.cases(combined_mid_data[, c("Species", "Genotype", col, "Treatment")]), ]
...
ggsave("middle_plot_labels2_star.pdf", final_plot, width = 20, height = 103, units = "cm")
