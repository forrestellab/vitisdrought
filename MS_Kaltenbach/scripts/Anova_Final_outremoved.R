...
#### INTERACTION EFFECT IS EXCLUDED because otherwise the anova wouldnt work with the outlier removal
# libraries ---------------------------------------------------------------
...
remove_outliers <- function(df, var) {
  ...
  df_filtered <- df
  df_filtered[[var]] <- ifelse(is.na(df[[var]]) | df[[var]] >= Q1 - 1.5 * IQR & df[[var]] <= Q3 + 1.5 * IQR, df[[var]], NA)
...
columns_to_filter_licor <- c("A", "Ci", "gsw", "E", "Tleaf", "intrWUE_AE", "instWUE_AGs")

# Columns to apply outlier removal
columns_to_filter_wp <- c("LWP", "PD", "SWP", "leafwp_predawn")
# Remove outliers by quartiles for each combination of species, treatment, and genotype
filtered_licor_df <- licor_final_df

for (col in columns_to_filter_licor) {
  filtered_licor_df <- filtered_licor_df %>%
    group_by(Species, Treatment, Genotype) %>%
    do(remove_outliers(., col)) %>%
    ungroup()
}

# Remove outliers by quartiles for each combination of species, treatment, and genotype
filtered_wp_df <- wp_final_df

for (col in columns_to_filter_wp) {
  if (any(!is.na(filtered_wp_df[[col]]))) {
    filtered_wp_df <- filtered_wp_df %>%
      group_by(Species, Treatment, Genotype) %>%
      do({
        if (nrow(.) > 0) {
          remove_outliers(., col)
...
wp_final_df <- filtered_wp_df
licor_final_df <- filtered_licor_df
...
write.csv(final_summary, file = "data_output/anova/final_summary_outremoved", row.names = FALSE)    
...
write.csv(species_final_anova_data, file = "data_output/anova/species_final_anova_data_outremoved", row.names = FALSE)
