...
# Combine All -------------------------------------------------------------
    ...
    beginning_summary <- full_join(A_summary, Ci_summary, by = c("Species", "Treatment")) %>%
    ...
    print(beginning_summary)
    
    write.csv(beginning_summary, file = "data_output/anova/beginning_summary", row.names = FALSE)    
...
# merge total -------------------------------------------------------------
    ...
    species_beginning_anova_data <- licor_beginning_df %>%
      ...
      left_join(beginning_summary, by = c("Species", "Treatment"))

    str(licor_beginning_df)
    str(beginning_summary)
    str(wp_beginning_df)
    duplicated(species_beginning_anova_data)
    
    
    
    
write.csv(species_beginning_anova_data, file = "data_output/anova/species_beginning_anova_data", row.names = FALSE)
