...
summary(anova)
porosity.cld <- multcompLetters4(anova_porosity, tukey_porosity)
cld <- as.data.frame.list(porosity.cld$`Species:Treatment_factor`)
porosity_summary$Tukey <- cld$Letters

# Convert the porosity_summary data frame to a regular data frame
porosity_summary_df <- as.data.frame(porosity_summary)

combined_data <- merge(segmentation_data, porosity_summary_df, by = c("Species", "Treatment"))

# new --------------------------------------------------------------------

porosity_box_plot <- ggplot(combined_data, aes(x = paste(Species), y = Porosity, fill = Treatment)) +
  geom_boxplot() +
  theme_bw() +
  theme(
  ...
  geom_text(data = combined_data %>% 
              group_by(Species, Treatment, Tukey) %>% 
              summarise(mean_Porosity = max(Porosity)),
            aes(x = paste(Species), y = mean_Porosity, label = Tukey),
