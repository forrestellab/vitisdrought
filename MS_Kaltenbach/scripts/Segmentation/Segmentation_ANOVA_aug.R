...
segmentation_data<- read.csv("data/segmentation_Aug.csv")
...
# run anovas --------------------------------------------------------------
    ...
    anova_SASP_SAWM <- aov(SASP_SAWM ~ Species + Treatment_factor + Species*Treatment_factor, 
                                    data = segmentation_data)
    tukey_SASP_SAWM <- TukeyHSD(anova_SASP_SAWM)
    SASP_SAWM.cld <- multcompLetters4(anova_SASP_SAWM, tukey_SASP_SAWM)
    cld_SASP_SAWM <- as.data.frame.list(SASP_SAWM.cld$`Species:Treatment_factor`)
    SASP_SAWM_summary$Tukey_SASP_SAWM <- cld_SASP_SAWM$Letters
    
    # Convert the Mesophyll_Width_summary data frame to a regular data frame
    SASP_SAWM_summary_df <- as.data.frame(SASP_SAWM_summary)
    

# SAPA_SAWM  --------------------------------------------------------------

    
    anova_SAPA_SAWM <- aov(SAPA_SAWM ~ Species + Treatment_factor + Species*Treatment_factor, 
                                    data = segmentation_data)
    tukey_SAPA_SAWM <- TukeyHSD(anova_SAPA_SAWM)
    SAPA_SAWM.cld <- multcompLetters4(anova_SAPA_SAWM, tukey_SAPA_SAWM)
    cld_SAPA_SAWM <- as.data.frame.list(SAPA_SAWM.cld$`Species:Treatment_factor`)
    SAPA_SAWM_summary$Tukey_SAPA_SAWM <- cld_SAPA_SAWM$Letters
    
    # Convert the Mesophyll_Width_summary data frame to a regular data frame
    SAPA_SAWM_summary_df <- as.data.frame(SAPA_SAWM_summary)
    
    

# SASPPA_SAWM -------------------------------------------------------------

    anova_SASPPA_SAWM <- aov(SASPPA_SAWM ~ Species + Treatment_factor + Species*Treatment_factor, 
                                    data = segmentation_data)
    tukey_SASPPA_SAWM <- TukeyHSD(anova_SASPPA_SAWM)
    SASPPA_SAWM.cld <- multcompLetters4(anova_SASPPA_SAWM, tukey_SASPPA_SAWM)
    cld_SASPPA_SAWM <- as.data.frame.list(SASPPA_SAWM.cld$`Species:Treatment_factor`)
    SASPPA_SAWM_summary$Tukey_SASPPA_SAWM <- cld_SASPPA_SAWM$Letters
    
    # Convert the Mesophyll_Width_summary data frame to a regular data frame
    SASPPA_SAWM_summary_df <- as.data.frame(SASPPA_SAWM_summary)
...
# Create combined File-------------------------------------------------------------------------
    ...
    write.csv(combined_seg_data, file = "data_output/segmentation/combined_seg_data_aug.csv", row.names = FALSE)
