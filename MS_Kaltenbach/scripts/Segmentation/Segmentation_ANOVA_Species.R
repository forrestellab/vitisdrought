...
# run anovas --------------------------------------------------------------
    ...
    cld_Porosity <- as.data.frame.list(Porosity.cld$`Species`)$Letters
    Porosity_combined_letters <- rep(cld_Porosity, each = length(unique(segmentation_data$Treatment_factor)))
    Porosity_summary$Tukey_Porosity <- Porosity_combined_letters
    ...
    cld_SpongyV_TotalMesophyllV <- as.data.frame.list(SpongyV_TotalMesophyllV.cld$`Species`)$Letters
    SpongyV_TotalMesophyllV_combined_letters <- rep(cld_SpongyV_TotalMesophyllV, 
                                                    each = length(unique(segmentation_data$Treatment_factor)))
    SpongyV_TotalMesophyllV_summary$Tukey_SpongyV_TotalMesophyllV <- SpongyV_TotalMesophyllV_combined_letters
    ...
    cld_PalisadeV_TotalMesophyllV <- as.data.frame.list(PalisadeV_TotalMesophyllV.cld$`Species`)$Letters
    PalisadeV_TotalMesophyllV_combined_letters <- rep(cld_PalisadeV_TotalMesophyllV, 
                                                      each = length(unique(segmentation_data$Treatment_factor)))
    PalisadeV_TotalMesophyllV_summary$Tukey_PalisadeV_TotalMesophyllV <- PalisadeV_TotalMesophyllV_combined_letters
    ...
    cld_SpongyV_PalisadeV<- as.data.frame.list(SpongyV_PalisadeV.cld$`Species`)$Letters
    SpongyV_PalisadeV_combined_letters <- rep(cld_SpongyV_PalisadeV, 
                                                      each = length(unique(segmentation_data$Treatment_factor)))
    SpongyV_PalisadeV_summary$Tukey_SpongyV_PalisadeV <- SpongyV_PalisadeV_combined_letters
    ...
    cld_Leaf_Width_um <- as.data.frame.list(Leaf_Width_um.cld$`Species`)$Letters
    Leaf_Width_um_combined_letters <- rep(cld_Leaf_Width_um, each = length(unique(segmentation_data$Treatment_factor)))
    Leaf_Width_um_summary$Tukey_Leaf_Width_um <- Leaf_Width_um_combined_letters
    ...
    cld_Mesophyll_Width_um <- as.data.frame.list(Mesophyll_Width_um.cld$`Species`)$Letters
    Mesophyll_Width_um_combined_letters <- rep(cld_Mesophyll_Width_um, each = length(unique(segmentation_data$Treatment_factor)))
    Mesophyll_Width_um_summary$Tukey_Mesophyll_Width_um <- Mesophyll_Width_um_combined_letters
...
# Create combined File-------------------------------------------------------------------------
    ...
    write.csv(combined_seg_data, file = "data_output/segmentation/species_seg_anova_data.csv", row.names = FALSE)
