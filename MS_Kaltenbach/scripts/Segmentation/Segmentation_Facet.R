...
combined_seg_data<- read.csv("data_output/segmentation/seg_anova.csv")
...
# Columns to plot
columns <- c("Porosity", "SpongyV_TotalMesophyllV", "PalisadeV_TotalMesophyllV", "SpongyV_PalisadeV", "Leaf_Width_um", "Mesophyll_Width_um", "SASP_SAWM", "SAPA_SAWM", "SASPPA_SAWM")
...
desired_order <- c("Porosity", "SpongyV_TotalMesophyllV", "PalisadeV_TotalMesophyllV", "SpongyV_PalisadeV", "Leaf_Width_um", "Mesophyll_Width_um", "SASP_SAWM", "SAPA_SAWM", "SASPPA_SAWM")
...
variable_labels <- c(
  ...
  `SASP_SAWM` = "SA[sp-cell]/~SA[mes-cell]~~~(μm^2~μm^-2)",
  `SAPA_SAWM` =  "SA[pa-cell]/~SA[mes-cell]~~~(μm^2~μm^-2)",
  `SASPPA_SAWM` = "SA[sp+pa-cell]/~SA[mes-cell]~~~(μm^2~μm^-2)"
)
...
final_plot <- ggplot(combined_data, aes(x = Species, y = Value, fill = Treatment)) +
  ...
  facet_wrap(
    ~Variable, scales = "free_y", ncol = 3, strip.position = "left",
...
# N=4 (ecxept control arizonica n=3)
