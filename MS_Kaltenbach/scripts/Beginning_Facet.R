...
combined_beg_data<- combined_beg_data %>%
  select(-starts_with("mean_"), -starts_with("sd_")) %>%
  select(-c(Treatment_factor, na.rm))%>%
  mutate(LWP = -LWP)   %>%
  distinct()
...
tukey_columns <- grep("^Tukey_", colnames(combined_beg_data), value = TRUE)

# Reshape the data to long format for Tukey values
tukey_data <- combined_beg_data %>%
...
print(summary_data, n=23)

#all n=7-8 but n= 1 for  of cinerea control, LWP n=2. no significance possible. 
print