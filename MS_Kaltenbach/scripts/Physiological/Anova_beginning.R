...
licor_beginning_df <- read.csv("data_output/Licor/licor_beginning")
wp_beginning_df <- read.csv("data_output/WP/wp_beginning")
...
wp_beginning_df <- wp_beginning_df %>% select(Species, Genotype, Treatment, LWP, PD)
...
wp_beginning_df_lwp <- wp_beginning_df[complete.cases(wp_beginning_df$LWP), ]
wp_beginning_df_pd <- wp_beginning_df[complete.cases(wp_beginning_df$PD), ]

wp_beginning_df_lwp <- wp_beginning_df[complete.cases(wp_beginning_df$LWP) & wp_beginning_df$Species != "vulpina", ]
...
# LWP ----------------------------------------------------------------
LWP_summary <- group_by(wp_beginning_df_lwp, Species, Treatment) %>%
  summarise(mean_LWP=mean(LWP, na.rm = TRUE), sd_LWP=sd(LWP, na.rm = TRUE)) %>%
  arrange(desc(mean_LWP))
print(LWP_summary)

# PD ----------------------------------------------------------------
PD_summary <- group_by(wp_beginning_df_pd, Species, Treatment) %>%
...
create.my.cld <- function(Y) {
  ...
  anova.Y <- aov(form, data = licor_beginning_df)
  ...
  rep(cld.Y, each = nlevels(licor_beginning_df$Treatment_factor))
}

my.aov <- function(Y) {
  form <- as.formula(paste(Y, "~ Species * Treatment_factor"))
  anova(aov(form, data = licor_beginning_df))
}
...
create.my.cld_lwp <- function(Y) {
  form <- as.formula(paste(Y, "~ Species * Treatment_factor"))
  anova.Y <- aov(form, data = wp_beginning_df_lwp)
  ...
  rep(cld.Y, each = nlevels(wp_beginning_df_lwp$Treatment_factor))
}

my.aov_lwp <- function(Y) {
  form <- as.formula(paste(Y, "~ Species * Treatment_factor"))
  anova(aov(form, data = wp_beginning_df_lwp))
}
...
beginning_summary_df <- full_join(A_summary_df, Ci_summary_df, by = c("Species", "Treatment")) %>%
...
write.csv(beginning_summary_df, file = "data_output/anova/beginning_summary", row.names = FALSE)    

# merge total -------------------------------------------------------------
species_beginning_anova_data <- licor_beginning_df %>%
  left_join(wp_beginning_df, by = c("Species", "Treatment", "Treatment_factor", "Genotype")) %>%
  left_join(beginning_summary_df, by = c("Species", "Treatment"))


write.csv(species_beginning_anova_data, file = "data_output/anova/anova_beginning", row.names = FALSE)
